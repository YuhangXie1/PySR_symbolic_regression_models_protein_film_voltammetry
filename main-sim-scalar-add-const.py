...
def calculate_percent_diff_to_set(model, capacitance_array):
    ...
    return percentage_diff_array[0], percentage_diff_array[1]
...
capacitance_array = [[np.random.uniform(-100.0,100.0),np.random.uniform(-100.0,100.0),0] for x in range(number_of_repeats)]
#capacitance_array = [[1,2,3] for x in range(number_of_repeats)]

output_filepath = rf"results/20250428-scalar-add-const-{percentage_noise*100}"
