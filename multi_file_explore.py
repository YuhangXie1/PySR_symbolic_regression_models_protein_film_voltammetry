...
def model(voltage, dv_dt, freq, current, time_data, ID):
    ...
    #X = np.array([voltage, voltage**2, voltage**3, dv_dt, dv_dt**2, dv_dt**3, freq]).T
    ...
    with open(os.path.join(output_filepath, "summary_current_model.csv"), "a", newline='') as file:
        ...
        #substituted_form = expand(expanded_form.subs([(x0,x),(x1,x**2),(x2,x**3),(x3,dx),(x4,dx**2),(x5,dx**3),(x6,f)]))
...
### main ###
output_filepath = rf"results/20250522-multi-fit-workflow-1/test 6/"
number_of_repeats = 3
files_freq = [9, 36, 45, 54, 63, 72, 81, 90, 99]

open_filepath = rf"results\20250522-multi-fit-workflow-1\test 5"

x0, x, dx, f = symbols("x0 x dx f")
file_voltage = pd.read_csv(os.path.join(open_filepath,"summary_voltage_models.csv"))
voltage_equations = sympify(np.array(file_voltage["voltage_eqn"]))
dv_dt_equations = sympify(np.array(file_voltage["dv_dt_eqn"]))
Hz_array = np.array(file_voltage["Hz"])
voltage_eqn_dict = {key:item for (key,item) in zip(Hz_array,voltage_equations)}
dv_dt_eqn_dict = {key:item for (key,item) in zip(Hz_array,dv_dt_equations)}

file_current = pd.read_csv(os.path.join(open_filepath,"summary_current_model.csv"))
current_equations = sympify(np.array(file_current["substituted_form"]))
...
freq = combined_data_array[:,4]

select_current_eqn = current_equations[0]
select_current_eqn_lambda = lambdify([x,dx,f],select_current_eqn)
predicted_current = select_current_eqn_lambda(voltage,dv_dt,freq)

#current time graph
fig, axs = plt.subplots(3)
axs[0].plot(time_data, current, label = "data", color = "blue")
axs[0].plot(time_data, predicted_current, label = "prediction", color = "red")
axs[0].set_xlabel("time")
axs[0].set_ylabel("current")
axs[0].set_title("Current vs time. Whole. ")
axs[0].legend()

slice_start = 10550
slice_end = 20000
axs[1].plot(time_data[slice_start:slice_end], current[slice_start:slice_end], label = "data", color = "blue")
axs[1].plot(time_data[slice_start:slice_end], predicted_current[slice_start:slice_end], label = "pred", color = "red", linestyle = "dotted")
axs[1].set_xlabel("time")
axs[1].set_ylabel("current")
axs[1].set_title(f"Current vs time. Slice {slice_start}-{slice_end}.")
axs[1].legend()

slice_start = 110550
slice_end = 120000
axs[2].plot(time_data[slice_start:slice_end], current[slice_start:slice_end], label = "data", color = "blue")
axs[2].plot(time_data[slice_start:slice_end], predicted_current[slice_start:slice_end], label = "pred", color = "red", linestyle = "dotted")
axs[2].set_xlabel("time")
axs[2].set_ylabel("current")
axs[2].set_title(f"Current vs time. Slice {slice_start}-{slice_end}.")
axs[2].legend()
...
axs.set_xlabel("dV/dt")
axs.set_ylabel("current")
axs.set_title(f"Current vs dV/dt.")
axs.legend()
