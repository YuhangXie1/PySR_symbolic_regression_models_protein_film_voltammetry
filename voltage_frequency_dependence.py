...
for Hz in files_freq:
    ...
    amplitude.append(abs(coeff_array[0]))
    frequency.append(coeff_array[1]/(2*np.pi))
    phase.append(abs(coeff_array[2]))
    offset.append(abs(coeff_array[3]))
...
v = voltage_old
w = 228.4492878688
ref_sin = np.sin(w * time_data_old)
ref_cos = np.cos(w * time_data_old)
...
fig, axs = plt.subplots()
axs.scatter(frequency, offset, label = "Offset")
axs.set_xlabel("frequency")
...
axs.scatter(frequency, phase, label = "Phase")
axs.set_xlabel("frequency")
