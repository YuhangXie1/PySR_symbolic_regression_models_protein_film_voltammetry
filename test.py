...
dx = dv_dt
x = voltage
A_val = combined_data_array[:,8]
p_val = combined_data_array[:,9]
w = combined_data_array[:,5]

current_pred = -0.008968267*A_val*dx - 0.69065799009977*A_val*p_val*x - 0.69065799009977*A_val*x**2 + 5.911939e-6*dx*p_val*x + 2.28552700361524e-6*dx*x + 1.289777e-5*dx + 0.0009932741637787*p_val*x - 0.00023674035*p_val + 5.911939e-6*w*x + 0.0009932741637787*x**2 - 9.4091796e-5

MSE = calculate_MSE(current, current_pred)
fig, axs = plt.subplots()
axs.plot(voltage, current, label = "real", color = "blue")
axs.plot(voltage, current_pred, label = "pred", color = "red")
axs.set_xlabel("voltage")
axs.set_ylabel("current")
axs.set_title(f"Files:{files_freq} - voltage-current - MSE: {MSE}")
axs.legend()
fig.tight_layout()
plt.show()
