...
def model(voltage, current, ID):
    ...
    with open(os.path.join(output_filepath, "summary_voltage_eqns.csv"), "a", newline='') as file:
        best = model.get_best()
    ...
    axs.plot(voltage, current, label = "data", color = "orange")
    axs.plot(voltage, predicted_current, label = "prediction", color = "magenta")
...
output_filepath = rf"results/20251204-ftv-voltage-eqn-1"
...
time_data, voltage, current = loading_data()
...
#loading data
data_volt = pd.read_csv(f"Data_for_eq_learning/FTacV_before_PSV_cv_voltage", sep="\t", names = ["time","voltage"])
data_amp = pd.read_csv(f"Data_for_eq_learning/FTacV_before_PSV_cv_current", sep="\t", names = ["time","current"])
...
for i in range(0,number_of_repeats):
    ...
    start_time = time.time()
    select_model = model(voltage, current, i)
    print (expand(select_model.sympy()))
    end_time = time.time()
...
axs.plot(time_data, current, label = "data", color = "orange")
#axs[0].plot(time_data, predicted_current, label = "prediction", color = "magenta")
axs.set_xlabel("time")
axs.set_ylabel("current")
axs.set_title(f"Current vs time. Whole. ")
axs.legend()
...
fig, axs = plt.subplots(1)
axs.plot(time_data, voltage, label = "data", color = "blue")
#axs[0].plot(time_data, predicted_current, label = "prediction", color = "magenta")
axs.set_xlabel("time")
axs.set_ylabel("voltage")
axs.set_title(f"Voltage vs time. Whole. ")
axs.legend()
fig.tight_layout()
plt.show()
...
fig, axs = plt.subplots(1)
axs.plot(voltage, current, label = "data", color = "blue")
#axs[0].plot(time_data, predicted_current, label = "prediction", color = "magenta")
axs.set_xlabel("voltage")
axs.set_ylabel("current")
axs.set_title(f"Voltage vs current. Whole. ")
axs.legend()
fig.tight_layout()
plt.show() """
