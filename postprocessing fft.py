...
for Hz in files_freq:
    output_filepath = rf"results/20250520-fit-workflow-fft-post-bandsize-0.5/{Hz}/"
    ...
    for eqn_number, eqn in enumerate(data_equations):
        ...
        x, dx = symbols("x dx")
        current_pred_lambda = lambdify([x, dx], eqn)
        ...
        axs.set_title(f"File {Hz}Hz, Eqn {eqn_number} all harmonics")
        axs.legend()
        fig.tight_layout()
        plt.savefig(os.path.join(output_filepath, f"eqn_{eqn_number}", f"Eqn-{eqn_number}-all-harmonic-freq.png"))
        ...
        for harmonic in range(1,10):
            ...
            axs.set_ylabel("Log10 ft^2")
            axs.set_title(f"File {Hz}Hz harmonic {harmonic} in freq domain")
            fig.tight_layout()
            Path(os.path.join(output_filepath, f"eqn_{eqn_number}","freq_domain")).mkdir(parents=True, exist_ok=True)
            plt.savefig(os.path.join(output_filepath, f"eqn_{eqn_number}","freq_domain", f"Eqn-{eqn_number}-harmonic-{harmonic}-freq.png"))
            ...
            fig.tight_layout()
            Path(os.path.join(output_filepath, f"eqn_{eqn_number}","time_domain")).mkdir(parents=True, exist_ok=True)
            plt.savefig(os.path.join(output_filepath, f"eqn_{eqn_number}","time_domain", f"Eqn-{eqn_number}-harmonic-{harmonic}-freq.png"))
            plt.close(fig.figure)
            ...
            fig.tight_layout()
            Path(os.path.join(output_filepath, f"eqn_{eqn_number}","voltage_domain")).mkdir(parents=True, exist_ok=True)
            plt.savefig(os.path.join(output_filepath, f"eqn_{eqn_number}","voltage_domain", f"Eqn-{eqn_number}-harmonic-{harmonic}-freq.png"))
            plt.close(fig.figure)
        ...
        fig1.tight_layout()
        fig1.savefig(os.path.join(output_filepath, f"eqn_{eqn_number}", f"Eqn-{eqn_number}-harmonics-freq-3x3.png"))
        ...
        fig2.tight_layout()
        fig2.savefig(os.path.join(output_filepath, f"eqn_{eqn_number}", f"Eqn-{eqn_number}-harmonics-time-3x3.png"))
        ...
        fig3.tight_layout()
        fig3.savefig(os.path.join(output_filepath, f"eqn_{eqn_number}", f"Eqn-{eqn_number}-harmonics-voltage-3x3.png"))
