...
for Hz in files_freq:
    load_filepath = rf"results\20250513-fit-workflow\{Hz}"
    ...
    MSE = current_summary_file["loss"]
    MSE_avg.append(float(np.average(MSE)))
    MSE_std.append(float(np.std(MSE)))

""" fig, axs = plt.subplots()
axs.bar(files_freq, MSE_avg, color = "blue")
#axs.errorbar(files_freq,MSE_avg,MSE_std)
fig.tight_layout()
plt.show()
 """
