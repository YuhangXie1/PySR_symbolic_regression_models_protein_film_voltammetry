...
class Simulator_poly3:

    def __init__(self, sim_equation, sim_coeff_array):
        self.sim_equation = sim_equation
        self.sim_coeff_array = sim_coeff_array
...
def model(x, y, sim_current, capacitance_array, ID):
    ...
    model = PySRRegressor(
        ...
        binary_operators=["+","*"],
        constraints={
        "*": [[(0, "constant"), (1, "variable")],[(0, "variable"), (1, "constant")]]
        },
        nested_constraints= {
            "*": {"*":0}
        },
...
def calculate_percent_diff_to_set(model, capacitance_array):
    ...
    try:
        coeff_array = [expression.coeff(x0,0).coeff(x1,0).coeff(x2,0), expression.coeff(x0,1), expression.coeff(x1,1), expression.coeff(x2,1)]
...
output_filepath = rf"results/20250501-poly-3-{percentage_noise*100 if add_noise else 0}-forced-poly-multiply-constrained-2"
