...
### main ###
output_filepath = rf"results/20250613-fit-multifreq"
load_filepath = "Data_for_eq_learning/20250611-CjX/20250611-blank-multifreq.csv"
...
voltage_pred_eqn = 0.3 * np.sin(2*np.pi*5*time_data) + 0.1 * np.sin(2*np.pi*7*time_data) + 0.05 * np.sin(2*np.pi*11*time_data)
...
A1 = np.full((1,len(time_data)), 0.3)[0]
A2 = np.full((1,len(time_data)), 0.1)[0]
A3 = np.full((1,len(time_data)), 0.05)[0]
w1 = np.full((1,len(time_data)), 2*np.pi*5)[0]
w2 = np.full((1,len(time_data)), 2*np.pi*7)[0]
w3 = np.full((1,len(time_data)), 2*np.pi*11)[0]
combined_data_array = np.array([time_data, voltage, dv_dt, current, A1, A2, A3, w1, w2, w3]).T
